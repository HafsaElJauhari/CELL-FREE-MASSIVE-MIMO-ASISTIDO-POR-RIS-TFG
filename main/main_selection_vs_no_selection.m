...
dist = 0:10:120;                       % Barrido simétrico de posiciones usuario
...
R_k_nosel_all     = zeros(length(dist), Iteration, length(frequencies), 2);  % K=2 usuarios
R_k_sel_all       = zeros(length(dist), Iteration, length(frequencies), 2);  % K=2 usuarios
...
K=2;                 % Dos usuarios en el escenario
P=4;                 % Subportadoras
R=2;                 % Número de RIS
...
for freq_idx = 1:length(frequencies)
    ...
    fprintf('\n=== Frecuencia: %.2f GHz ===\n', frequency/1e9);
    fprintf('Comparación (Selección vs. Sin selección)\n');
...
% ----- 2) Inicialización común de W y Theta -----
            ...
            R_k_nosel_all(a,b,freq_idx,:) = R_k_nosel;  % Tasas individuales por usuario (sin selección)
            R_k_sel_all(a,b,freq_idx,:)   = R_k_sel;    % Tasas individuales por usuario (con selección)
        end
    end
end
...
% Promedios de tasas individuales por usuario
...
for freq_idx = 1:length(frequencies)
    ...
    % Promedio de tasas individuales por usuario
    for k = 1:K
...
toc
save('resultados_selection_vs_no_selection.mat');
fprintf('Resultados guardados en: resultados_selection_vs_no_selection.mat\n');

%% Función auxiliar: posiciones del escenario
...
user_position(2, :) = [120-current_dist 0];
