function [W,R_sum]=MyAlgorithm_noRIS(B,BS_antennas,User_antennas,P_max,K,P,R,N_ris,sigma2,H_bkp,F_rkp,G_brp,W)
iteration=30;  %��������
R_sum=zeros(2*iteration,1);
gamma_k=ones(K,1);      %�û����ȼ�
alpha_kp=ones(K,P);     %��������alpha 
beta_kp=ones(K,P);      %��������beta
Theta_r=zeros(R,N_ris,N_ris);                     %��ͬRIS�ϵ���λ
%w_bpk=sqrt(P_max/K/P/BS_antennas/2)*(ones(B,P,K,BS_antennas)+1j*ones(B,P,K,BS_antennas));     %����Ԥ�������
%%  Լ�������ϲ�
...
if Q>1
    if (R_sum(2*Q-1)-R_sum(2*Q-3))/R_sum(2*Q-2)<0.01   %������С��0.5%�򵯳�
       break; 
    end
end
% AAA=R_sum(Q)
...
% plot(abs(R_sum));
R_sum=max(R_sum(2*Q),R_sum(2*Q-1));
end